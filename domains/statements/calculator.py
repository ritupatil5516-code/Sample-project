...
def total_interest(stmts: List[dict], account_id: Optional[str], period: Optional[str]):
    ...
    return {'interest_total': round(total,2), 'trace': {'count': len(match), 'period': period, 'close_date': match[-1].get('closingDateTime') or match[-1].get('closing_date_time')}}
...
def interest_breakdown(stmts: List[dict], account_id: Optional[str], period: Optional[str]):
    ...
    return {'interest_total': total, 'non_trailing': non_trailing, 'trailing': trailing, 'trace': {'period': period, 'close_date': stmt.get('closingDateTime') or stmt.get('closing_date_time')}}
...
def trailing_interest(stmts: List[dict], account_id: Optional[str], period: Optional[str]):
    ...
    stmt = match[-1]
    trailing = float(stmt.get('totalTrailingInterest') or stmt.get('total_trailing_interest') or 0)
    return {'trailing_interest': trailing, 'trace': {'period': period, 'close_date': stmt.get('closingDateTime') or stmt.get('closing_date_time')}}
